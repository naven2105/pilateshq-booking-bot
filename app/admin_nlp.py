...
def parse_admin_client_command(text: str) -> dict | None:
    ...
    if m: return {"intent":"cancel_next","name":m.group(1).strip()}

    m = re.match(r'(?i)^\s*(.+?)\s+is\s+off\s+sick\.?\s*$', s)
    if m: return {"intent":"off_sick_today","name":m.group(1).strip()}

    m = re.match(r'(?i)^\s*(.+?)\s+is\s+no\s+show\s+today\.?\s*$', s)
    if m: return {"intent":"no_show_today","name":m.group(1).strip()}
