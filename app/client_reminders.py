...
All sends use pre-approved WhatsApp templates (HSM) to bypass the 24h window.
Templates are expected to accept a single {{1}} body parameter (one big text).
"""
...
# ── Template names (configurable via env-backed config.py if you prefer) ─────
...
def _send_client_template(wa_number: str, template_name: str, text_body: str) -> bool:
    """Send a one-body-param template to a client WA number."""
    if not wa_number:
        return False
    try:
...
def run_client_tomorrow() -> int:
    ...
    Uses template `session_tomorrow` with one text body param.
...
def run_client_next_hour() -> int:
    ...
    Uses template `session_next_hour` with one text body param.
    """
    now = datetime.now()
...
def run_client_weekly() -> int:
    ...
    Uses template `client_weekly` with a consolidated text body.
