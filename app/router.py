...
def webhook():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # ── Interactive replies (flows, nfm, buttons) ──
            if msg.get("type") == "interactive":
                ...
                if itype in {"flow_reply", "nfm_reply"}:
                    ...
                    if client_name and mobile:
                        ...
                        send_whatsapp_text(from_wa, "⚠️ Client form could not be parsed.")

                    return jsonify({"status": "ok", "role": itype}), 200

                # Button replies (from interactive templates)
                elif itype == "button_reply":
                    button = interactive.get("button_reply", {})
                    button_id = button.get("id") or ""
                    btn_key = button_id.lower()
                    ...
                    if btn_key.startswith("reject_"):
                        ...
                        with get_session() as s:
                            s.execute(
                                text("UPDATE bookings SET status='rejected' WHERE session_id=:sid AND wa_number=:wa"),
                                {"sid": sid, "wa": normalize_wa(from_wa)},
                            )
                        # Confirm to client
                        safe_execute(send_whatsapp_text, from_wa,
                            "❌ Your booking has been cancelled. Nadine has been notified.",
                            label="client_reject_ok"
                        )
                        # Alert Nadine
                        admin_nudge.notify_cancel("Client", from_wa, f"session {sid} rejected by client")
            ...
            # ── Plain Button (Meta type=button) ──
            if msg.get("type") == "button":
                button = msg.get("button", {})
                button_id = button.get("payload") or button.get("text") or ""
                btn_key = button_id.lower().replace(" ", "_")
                handle_admin_action(from_wa, msg.get("id"), None, btn_id=btn_key)
                return jsonify({"status": "ok", "role": "admin_button"}), 200
