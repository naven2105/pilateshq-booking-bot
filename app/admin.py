...
# State shape (per-admin WA):
# {
#   "flow": "NEW" | "UPDATE" | "CANCEL" | "VIEW" | "BOOK" | None,
#   "awaiting": "NAME" | "PHONE" | "DOB" | "PLAN" | "MEDICAL" | "CREDITS" | None,
#   "cid": int | None,
#   "buffer": dict,
#   "book": dict
# }
...
def _get_state(wa: str) -> dict:
    ...
    if not st:
        st = {"flow": None, "awaiting": None, "cid": None, "buffer": {}, "book": {}}
        ADMIN_STATE[wa] = st
    return st

def _set_state(wa: str, *, flow=None, awaiting=None, cid=None, buffer=None, book=None):
    st = _get_state(wa)
    if flow is not None:
        st["flow"] = flow
...
def handle_admin_action(sender: str, text: str):
    ...
    raw = (text or "").strip()
    up = raw.upper()
    logging.info(f"[ADMIN CMD] '{raw}' (flow={state['flow']} awaiting={state['awaiting']} cid={state['cid']})")

    # ── Intent shortcuts (interactive buttons)
    if up == "ADMIN_INTENT_NEW":
        _set_state(wa, flow="NEW", awaiting=None, cid=None, buffer={}, book={})
        return _start_new(wa, _get_state(wa))

    if up == "ADMIN_INTENT_UPDATE":
        _set_state(wa, flow="UPDATE", awaiting=None)
        return _client_picker(wa, "Update: pick client")

    if up == "ADMIN_INTENT_CANCEL":
        _set_state(wa, flow="CANCEL", awaiting=None)
        return _client_picker(wa, "Cancel: pick client")

    if up == "ADMIN_INTENT_VIEW":
        _set_state(wa, flow="VIEW", awaiting=None)
        return _client_picker(wa, "View: pick client")

    if up == "ADMIN_INTENT_BOOK":
        _set_state(wa, flow="BOOK", awaiting=None)
    ...
    if up.startswith("ADMIN_PICK_CLIENT_"):
        ...
        flow = _get_state(wa)["flow"]
        if flow == "VIEW":
            return _show_profile(wa, cid)
        if flow == "UPDATE":
            return _update_menu(wa, cid)
        if flow == "CANCEL":
            # placeholder unless you have a CRUD helper wired
            return send_whatsapp_text(wa, "Cancel-by-client (stub): add a CRUD helper to cancel next booking.")
        if flow == "BOOK":
...
def _start_new(wa: str, state: dict):
    _set_state(wa, awaiting="NAME", buffer={}, cid=None)
